...
pub fn collect_trait_impls(krate: Crate, cx: &DocContext<'_>) -> Crate {
    ...
    for def_id in primitive_impls.iter().filter_map(|&def_id| def_id) {
        if !def_id.is_local() {
            ...
            if !cx.tcx.get_attrs(def_id).lists(&cx.tcx.sess.parse_sess, sym::doc).has_word(sym::hidden) {
...
impl<'a, 'tcx> DocFolder for SyntheticImplCollector<'a, 'tcx> {
    fn fold_item(&mut self, i: Item) -> Option<Item> {
        if i.is_struct() || i.is_enum() || i.is_union() {
            // FIXME(eddyb) is this `doc(hidden)` check needed?
            if !self.cx.tcx.get_attrs(i.def_id).lists(&self.cx.tcx.sess.parse_sess, sym::doc).has_word(sym::hidden) {
