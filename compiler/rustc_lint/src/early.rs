...
fn early_lint_module<T: EarlyLintPass>(
    sess: &Session,
    ...
    cx.with_lint_attrs(ast::CRATE_NODE_ID, &module.attrs, |cx| {
        // since the root module isn't visited as an item (because it isn't an
        // item), warn for it here.
        run_early_pass!(cx, check_crate, module);

        ast_visit::walk_crate(cx, module);

        run_early_pass!(cx, check_crate_post, module);
    });
    cx.context.buffered
}

pub fn check_ast_module<T: EarlyLintPass>(
    sess: &Session,
    pre_expansion: bool,
    lint_store: &LintStore,
    registered_tools: &RegisteredTools,
