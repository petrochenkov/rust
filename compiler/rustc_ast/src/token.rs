...
pub enum InvisibleSource {
    ...
    ExprMv,
    // Converted from `TokenKind::Interpolated` in `FlattenNonterminals`.
    // njn: may be mergeable with `ProcMacro`.
    FlattenNonterminals,
    ...
    pub fn skip(&self) -> bool {
        match self {
            Delimiter::Invisible(src) => match src {
                InvisibleSource::FlattenNonterminals | InvisibleSource::ProcMacro => true,
...
impl Token {
    ...
    pub fn can_begin_const_arg(&self) -> bool {
        match self.kind {
            // njn: can_begin_literal_maybe_minus also checks for ExprMv
    ...
    // petrochenkov: Likely yes, it's used in is_pat_range_end_start which is
    // observable externally.
    // petrochenkov [later]: In case of is_pat_range_end_start it's already
    // included into can_begin_literal_maybe_minus, and the only other use if
    // for diagnostics, so it can be dropped. I'd remove this whole function,
    // the whole is_whole_expr check doesn't fit into the new token-based
    // model.
