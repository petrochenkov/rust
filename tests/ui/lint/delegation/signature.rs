...
impl Hash for NonZeroU32 {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.0.hash(state);
        //~^ ERROR dstats. stmts: OneWithoutTail, args_match: Same, ret_match: Same, self_arg: Value, same_name: true, has_expr_after: false.
    }
}
...
impl Add for NonZeroU32 {
    ...
    fn add(self, other: Self) -> Self {
        ...
        //~^ ERROR dstats. stmts: ZeroWithTail, args_match: SameNumber, ret_match: Same, self_arg: Type, same_name: false, has_expr_after: false.
        //~| ERROR dstats. stmts: ZeroWithTail, args_match: SameUpToSelfType, ret_match: SameUpToSelfType, self_arg: Value, same_name: true, has_expr_after: true.
    }
}
...
mod anon1 {
    ...
    impl Struct {
        pub fn new() -> Box<dyn Trait> {
            Box::new(Struct)
            //~^ ERROR dstats. stmts: ZeroWithTail, args_match: Different, ret_match: Same, self_arg: Type, same_name: true, has_expr_after: false.
        }
...
mod anon2 {
    ...
    fn foo<'a>(f: &F<'a>) -> &'a dyn Display {
        f.foo()
        //~^ ERROR dstats. stmts: ZeroWithTail, args_match: Different, ret_match: Same, self_arg: Value, same_name: true, has_expr_after: false.
    }
}
